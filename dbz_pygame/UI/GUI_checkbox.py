...
class CheckBox(Widget):
            ...
            for evento in lista_eventos:
                if evento.type == pygame.MOUSEBUTTONDOWN:
                    if self.slave_rect_collide.collidepoint(evento.pos):
                        self.__esta_prendido = not self.__esta_prendido
                        self.contador_click = 0
        else:
            self.contador_click += 1

        if self.__esta_prendido:
    ...
    def get_esta_prendido(self):
        '''getter esta prendido'''
        return self.__esta_prendido
