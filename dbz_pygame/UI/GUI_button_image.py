...
class ButtonImage(Widget):
    '''clase button image'''
    def __init__(self, screen:pygame.Surface, master_x:int, master_y:int, x:int, y:int, w:int, h:int, path_image:str, onclick=None, onclick_param=None, text="", font="Avenir Next", font_size=12, font_color="Black", color_background=None, color_border="Black", border_size=-1):
            ...
            for evento in lista_eventos:
                if evento.type == pygame.MOUSEBUTTONDOWN:
                    if self.slave_rect_collide.collidepoint(evento.pos):
                        if self._onclick is not None and self._onclick_param is not None:
